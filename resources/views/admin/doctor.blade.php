            ...
            <div class="modal-content bg-light-pink" style="padding: 50px;">
                <form id="editForm{{ $service->id }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-3">
                        ...
                        <img id="previewImage1" src="{{ asset('storage/' . $service->foto) }}" alt="Preview Image"
                        ...
                        <div class="d-flex">
                            <div class="d-block w-100">
                                ...
                                <div class="d-flex overflow-x-auto w-100">
                                    @foreach(['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu', 'minggu'] as $day)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input form-check-input-doctor" type="checkbox"
                                                value="{{ ucfirst($day) }}" id="flexCheck{{ ucfirst($day) }}" name="jadwal[]" {{ in_array($day, $service->jadwal) ? 'checked' : '' }}>
                                            <label class="form-check-label"
                                                for="flexCheck{{ ucfirst($day) }}">{{ ucfirst($day) }}</label>
                                        </div>
                                    @endforeach
                                </div>
                            </div>
